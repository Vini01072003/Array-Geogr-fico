// Criação da váravel que vai receber um conjunto de Jsons dentro do Array
var estadosCidades = {
    pais: 'Brasil',
    estados: [{
        sigla: 'SP',
...
// Criação da constante que vai pegar uma lista de Estados na função, em que será criado um Json e um array com lista de estados
const getListaDeEstados = function () {
    let estadoCidadeJson = {}
    let listaEstados = []

    estadosCidades.estados.forEach(estadoFor => {

        listaEstados.push(estadoFor.sigla)
        estadoCidadeJson = listaEstados
    })

    return estadoCidadeJson
}
...
const getDadosEstado = function () {
    let listDadosDoEstado = {}

    estadosCidades.estados.forEach(function (estado) {
        listDadosDoEstado = { uf: estado.sigla, nome: estado.nome, capital: estado.capital }

    })
    return listDadosDoEstado
...
function getCapitalEstado(siglaEstado) {
    ...
    estadosCidades.estados.forEach(estados => {
        if (siglaEstado == estadosCidades.sigla) {
            listaDadosDoEstado.uf = estados.sigla
            listaDadosDoEstado.descrição = estados.nome
...
function getEstadosRegiao(regiaopais) {
    let JsonEstados = {}
    let visualizarestados = []

    estadosCidades.estados.forEach(function (estado) {
        if (estado.regiao == regiaopais) {
            visualizarestados.push(estado.sigla, estado.nome, estado.capital)
        }
    })
    JsonEstados = { regiao: regiaopais, estado: visualizarestados }
    return JsonEstados
